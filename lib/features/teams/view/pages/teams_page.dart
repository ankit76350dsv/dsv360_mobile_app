...
class _TeamsPageState extends ConsumerState<TeamsPage> {
        ...
        _employees = batchProfiles
            .map(
              (profile) => Employee(
                ...
                phone: profile.phone ?? '',
                profileImageUrl: profile.profilePic.isNotEmpty
                    ? profile.profilePic
                    : null,
  ...
  List<Employee> get _unassigned {
    ...
    return all
        .where((e) => e.name.toLowerCase().contains(q) || e.phone.contains(q))
        .toList();
  }

  int get _totalUnassigned => _employees.where((e) => e.teamId == null).length;
  ...
  Future<void> _moveEmployee(
    ...
    try {
      ...
      showErrorSnackBar(
        context,
        'Failed to save team assignment. Please try again.',
      );
    }
  }
  ...
  void _confirmDeleteTeam(Team team) {
    ...
    if (teamMembers.isNotEmpty) {
      ...
      if (mounted) {
        showErrorSnackBar(
          context,
          'Cannot delete team with members. Please unassign all members first.',
        );
      }
      return;
    }
